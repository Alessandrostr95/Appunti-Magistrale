...
** Coordination Game - Modello Lineare
   ...
   Perciò possiamo dire a $v$ conviene cambiare stato se
   \begin{align*}
     n_A a &\geq n_B b\\
...
** Configurazioni d'equilibrio
   Cerchiamo ora di capire quali sono le possibili configurazioni di equilibrio della rete, ovvero configurazioni di stati =A-B= in cui nessuno più cambia stato.
   Certamente due configurazioni banali sono la configurazione in cui nessuno adotta =A=, e quindi sono tutti nello stato =B=, oppure la configurazione in cui, una volta introdotto =A=, tutti passano
   allo stato =A=.\\
