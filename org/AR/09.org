...
** Cascate e Cluster
   ...
   Riguardando l'[[fig:img7][esempio 2]], possiamo individuare delle componenti più coese di nodi, più precisamente
   ...
   *Def (Cluster di densità $p$)*
   ...
   Osservare che una [[./07.html][strong web community]] è un cluster di densitatà $p$ con \(p = \frac{1}{2}\).
   ...
   *THM*
   ...
   L'insieme \(V_0\) *non* genera una cascata completa di =A= _se e solo so_ \(G-V_0\) congiene un /cluster di densità/ \(1 - q\).\\

   *Proof*
   Iniziamo col dimostrare il primo verso dell'implicazione, ovvero che se \(V_0\) non genera una cascata completa di =A= *allora* \(G-V_0\) congiene un /cluster di densità/ \(1 - q\).\\
   ...
   Prendiamo uno dei nodi \(u \in C\) che passano in =A= al tempo $t$, ovvero un \(u \in V_t\).
   Poiché $C$ è un cluster di densità \(1-q\) avremo che
   \[
   ...
   Ricordiamo dalle lezioni precedenti ([[./07.html][Communities - Part 1]] e [[./08.html][Communities - Part 2]]) che generalmente i legami che colleggano due comunità (o cluster) sono dei *legami deboli* (*weak ties*).
   ...
   Pertanto forniscono accesso a fonti di informazioni (per esempio nuove opportunità di lavoro[fn:1]) che risiedono in parti della rete a cui altrimenti non avremmo accesso.\\
   ...
   Infatti considerando ancora l'[[fig:img7][esempio 2]], possiamo osservare che i nodi 4 e 9 vengono subito a conoscenza dell'innovazione =A= in quanto entrambi sono vicini di un nodo iniziatore,
   però attendono più tempo prima di adottarlo.
   Anche in un contesto sociale reale, sebbene una barzelletta o un video divertente online avanzano con una velocità notevole, la mobilitazione politica si muove più lentamente,
...
** Cluster e Marketing Virale
   ...
   - aumentare il guadagno ricavato dall'adottare =X= aumentando il rapporto /qualità/prezzo/, o abbassando il prezzo o aumentando la qualità.
     In genere però non è sempre possibile abbassare troppo i prezzi di un prodotto.
     Da un lato perché il venditore andrebbe a prendere, dall'altro perché in un contesto sociale un prodotto con prezzo troppo basso è spesso sinonimo di scarsa qualità.
   ...
   Perciò una domanda che ci si pone è: scegliendo appositamente gli iniziatori, quanto alta posso mantenere la soglia di adozine di una innovazione?
...
* The Cascade Capacity on Infinite Network with bounded Degree
  In questa sezione analizzeremo in maniera più formale, tramite l'aiuto di alcuni esempi, se esiste e qual è la /threshold/ (per una soglia di adozione) che scatena una cascata completa,
  anche con un insieme "piccolo" di nodi /iniziatori/.\\
  Più in particolare analizzeremo il caso di reti con *infiniti nodi*, ma ognuno con *grado finito*.
  In questa maniera il concetto di /"insieme piccolo di iniziatori"/ è definito da qualsiasi *finito*.\\

  #+begin_quote
  *Problem*
  Dato un grafo *infinito* \(G = (\mathbb{N}, E)\), dove tutti i nodi hanno un *grado finito*, trovare la /soglia di adozione massima/ \(q_{MAX}\) di $G$ tale che _esiste_ un sottoinsieme
  finito \(V_0 \subset \mathbb{N}\) di iniziatori che genera una cascata completa.
  #+end_quote
...
** Esempio 1 - Catena infinita
   ...
   #+caption: Cascata completa con \(V_0 = \lbrace x \rbrace\) e \(q = \frac{1}{2}\).
   #+ATTR_HTML: :style max-width:550px; :width 100%
   [[file:../images/ar-lesson09-img11.gif]]
   ...
   #+caption: Blocco della cascata completa per \(V_0 = \lbrace w,y,z \rbrace\) e \(q > \frac{1}{2}\).
   #+ATTR_HTML: :style max-width:550px; :width 100%
   [[file:../images/ar-lesson09-img12.gif]]


** Esempio 2 - Griglia
   Consideriamo ora una *griglia* infinta \(\mathbb{Z} \times \mathbb{Z}\), dove ogni nodo è collegato coi nodi nelle rispettive posizione =UP=, =DOWN=, =LEFT=, =RIGHT=, =NE=, =NW=, =SE= e =SW=,
   come nella seguente figura
   
   #+caption: Griglia infita, dove ogni nodo ha grado esattamente 8.
   #+ATTR_HTML: :style max-width:550px; :width 100%
   [[file:../images/ar-lesson09-img13.png]]

   Consideriamo il caso base in cui \(\vert V_0 \vert = 1\).
   In tal caso la soglia di adozione deve essere _al più_ \(q \leq \frac{1}{8}\).
   Con una soglia maggiore non si riuscirebbe altrimenti a creare una cascata completa.
   
   #+caption: Cascata completa con \(\vert V_0 \vert = 1\) e soglia d'adozione \(q \leq \frac{1}{8}\).
   #+ATTR_HTML: :style max-width:550px; :width 100%
   [[file:../images/ar-lesson09-img14.gif]]
   ...
   #+caption: Cascata completa con \(\vert V_0 \vert = 2\) e soglia d'adozione \(q = \frac{1}{4}\).
   #+ATTR_HTML: :style max-width:550px; :width 100%
   [[file:../images/ar-lesson09-img15.gif]]


   Anche con \(\vert V_0 \vert = 3\) si riesce ad aumentare la soglia di adozione del colore blu a \(q = \frac{3}{8}\).
   Basta ancora una volta scegliere come iniziatori tre nodi adiacenti.

   #+caption: Cascata completa con \(\vert V_0 \vert = 3\) e soglia d'adozione \(q = \frac{3}{8}\).
   #+ATTR_HTML: :style max-width:550px; :width 100%
   [[file:../images/ar-lesson09-img16.gif]]
   ...
   Infatti è facile convincersi che, qualsiasi sottoinsieme finito di iniziatori prendiamo, si arriverà prima o poi ad una situazione in cui si sarà /quasi/ o del tutto colorata di blu un'area rettangolare,
   dalla quale non si può uscire a meno che non si ha \(q \leq \frac{3}{8}\).
   
   #+caption: Controesempio con \(\vert V_0 \vert > 3\) e soglia d'adozione \(q = \frac{1}{2}\).
   #+ATTR_HTML: :style max-width:550px; :width 100%
   [[file:../images/ar-lesson09-img17.gif]]

   È infatti possibile dimostrare che per questa griglia infinita la /capacità di cascata/  \(q_{MAX}\) è proprio \(\frac{3}{8}\).
   
   Continuo nella [[./10.html][lezione successiva →]]
   
[fn:1] Esperimento di Granovetter.
