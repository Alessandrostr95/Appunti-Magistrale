...
**** *Maggiorazione per* $\sum_{j \in \left[ n \right] \setminus \lbrace i \rbrace} \mathcal{P}\left( \mathcal{E}_{i,j} \right)$
	...
	[[file:../images/ar-lesson04-img7.png]]
     ...
     Per quanto riguarda $\mathcal{P}(\mathcal{E}^1_{i,j})$ è abbastanza semplice, in quanto la probabilità che un singono $h$ *non* appartenga all'area
     ...
     \end{align*}
     Dove $f(t) = \frac{1}{|Q|} = 1$ ricordiamo essere la funzione di densità riguardo la scelta di un punto uniformemente a caso in $Q$.\\

     
     Più articolato è il calcolo di $\mathcal{P}(\mathcal{E}^1_{i,j})$, dato che
     \[|C_r(t_i) \cup C_r(t_j)| = |C_r(t_i)| + |C_r(t_j)| - |C_r(t_i) \cap C_r(t_j)| = 2\pi r^2 - |C_r(t_i) \cap C_r(t_j)|\]
     in quanto si necessita quindi di calcolare $|C_r(t_i) \cap C_r(t_j)|$.
     Dato che non si può determinare a pripriori $|C_r(t_i) \cap C_r(t_j)|$ in qunato un evento casuale, sa stiemarà il valore /minimo/ che
     $|C_r(t_i) \cap C_r(t_j)|$ può assumere.\\
