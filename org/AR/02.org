...
*Teorema del limite centrale*\\
    ...
    #+attr_html: :width 100% :style max-width: 400px;
...
** Riconoscere una power law 
   ...
   All'atto pratico, invece di considerare la funzione $y = f(x)$, verrà mostrata la funzione $\log{(y)} = \log{\left( f(x) \right)}$. 
    ...
    #+attr_html: :width 100% :style max-width: 600px;
...
**** *Individuazione Power Law*
     ...
     Perciò la frazione dei nodi con grado $k$ sarà la differenza tra il numero di nodi con grado _almeno_ $k$ e il numero di nodi
     ...
     Ma sappiamo dalla precedente eq. differenziale che $x_j(t) = \frac{p}{1-p} \left[ \left( \frac{t}{j}  \right)^{1-p} - 1 \right]$,
     perciò $x_j(t) \geq k$ se e solo se $\frac{p}{1-p} \left[ \left( \frac{t}{j} \right)^{1-p} - 1 \right] \geq k$.\\
     ...
     Assodata questa approssimazione, si può procedere derivando[fn:7] in $k$.
     \begin{align*}
     ...
     Tale funzione è l'inversa di un polinomio in $k$ con esponente massimo $\left( 1 + \frac{1}{1-p} \right)$,
     e in quanto tale è una /power-law/.
...
*** Osservazioni sull'analisi
    ...
    In realtà è stato dimostrato[fn:8] che /w.h.p./[fn:9] nel modello di rete per /rich-get-richer/ proposto, la frazione di nodi
...
* Impredicibilità della popolarità
  ...
  C'è un esperimento di [[https://www.princeton.edu/~mjs3/salganik_dodds_watts06_full.pdf][Salganik-Dodds-Watts]] del 2006 che mostra quanto è importante e instabile la fase iniziale.
  #+begin_quote
  Venne creato un sito era possibile ascoltare e scaricare delle canzoni tra circa 50 proposte.
  C'erano inoltre 8 playlist, ognunga delle quali contente tutte le canzoni del sito.
  Per ogni playlist, veniva mostrato un /contatore dei download/ delle canzoni internamente alla playlist:
  se la canzone =X= veniva scaricata dalla playlist =A=, quel download veniva contato e riportato solo per =A=, e non per un'altra playlist =B=.\\
  ...
  Dato che inizialmente erano tutti posti a 0, e dato che le persone venivano rendirizzate alle playlist _totalmente a caso_, si potrebbe pensare
  che col tempo più o meno tutte le playlist avessero una stessa classifica dei download.
...
* La lunga coda
  La distribuzione della popolarità dei nodi di una rete può avere importanti effetti su attività commerciali.
  Per esempio consideriamo un sito di vendita di libri online.
  Il gestore del sito potrebbe chiedersi:
  #+begin_quote
  /è più conveniente vendere _tante copie di un pochi libri molto popolari_, oppure è meglio vendere _poche copie ma di tantissimi libri poco popolari_?/
  #+end_quote
  ...
  Come prima cosa enumeriamo i libri a disposizione in un ordine _non crescente_ di popolarità.
  Ovvero, siano i libri \(\{ 1, ..., n \}\) disponibili nel magazzino online, assumiamo che essi sono ordinati in modo tale che la popolarità (o grado entrante) \(\delta(i)\) del libro $i$ è 
  _non maggiore_ di quella di \(i+1\).
  \[
  \delta(1) \geq \delta(2) \geq ... \geq \delta(n)
  \]
  # scatter(books, p, label=:none, c=:red, markersize=6, xlabel="books", ylabel="in-degree", xticks=1:16, yticks=0:5, ylim=(0,5.5))
  #+ATTR_HTML: :width 100% :style max-width: 500px;
  [[file:../images/ar-lesson02-img3.png]]

  A questo punto, assumiamo che il numero di nodi avente una certa popolarità _aumenti_ al diminurie della popolairtà.
  Ovvero i il numero di nodi con popolarità 100 sarà _minore_ di quelli con popolarità 99, che a sua volta sarà pià piccolo di quelli con popolairtà 98, ecc...\\
  ...
  # scatter(nbooks, p, label=:none, c=:blue, markersize=7, xlabel="books' quantity", ylabel="in-degree", xticks=1:16, yticks=0:5, ylim=(0,5.5), shape=:rect)
  #+ATTR_HTML: :width 100% :style max-width: 500px;
  #+NAME: fig:grafico1
  [[file:../images/ar-lesson02-img4.png]]
  ...
  # scatter(p, nbooks, label=:none, c=:blue, markersize=7, ylabel="books' quantity", xlabel="in-degree", yticks=0:6, xticks=1:5, ylim=(0,6.5), shape=:rect)
  #+CAPTION: Grafico con assi invertite.
  #+NAME: fig:grafico2
  #+ATTR_HTML: :width 100% :style max-width: 500px;
  [[file:../images/ar-lesson02-img5.png]]

  Indichiamo formalmente con \(\#(k) = n\) la funzione che data la popolarità $k$ ci dice il numero di libri $n$ con tale probabilità, come quello mostrato in figura [[fig:grafico2]].\\

  Quello che interessa a noi è identificare la funzione che descrive il grafico [[fig:grafico1]], ovvero la funzione \(\#^-1(n) = k\) che dato un numero di libri $n$ aventi la stessa popolarità ci dice la loro popolarità esatta $k$.
  Sempre ammettendo che la funzione \(\#(\cdot)\) sia _invertibile_.\\
  ...
  Se quindi \(k \approx n^{-1/c}\), dove $k$ è un livello di popolarità ed $n$ è il numero di nodi con tale popolarità, possiamo osservare graficamente
  che $k$ decresce *mooolto* lentamente.
  
  # plot(1:10^5, n -> 100*n^(-.2), xticks=(0:2*10^4:10^5, ["1", "2x10^4", "4x10^4", "6x10^4", "8x10^4", "10^5"]), yticks=0:10:100, ylims=(0,110), label=:none, w=2, xlabel="books' quantity", ylabel="in-degree")
  # plot(3:100, n -> 100*n^(-.3), xticks=(3:20:100, ["1", "2x10^3", "4x10^3", "6x10^3", "8x10^3", "10^5"]), yticks=0:10:100, ylims=(0,110), label=:none, w=2, xlabel="books' quantity", ylabel="in-degree")
  #+NAME: fig:grafico3
  #+ATTR_HTML: :width 100% :style max-width: 500px;
  [[file:../images/ar-lesson02-img6.png]]
  
  Come possiamo vedere dalla Figura [[fig:grafico3]], la popolarità $k$ al crescere del numero dei libri decresce molto lentamente.
  ...
  l'area di quelli poco popolari, ovvero l'area sotto la *lunga coda*, è _tutt'altro che trascurabile_.\\
...
[fn:7] si ricordi che la derivata di una funzione del tipo $y = \left[ h(x) \right]^n$ è pari a $y' = n \cdot \left[ h(x) \right]^{n-1} \cdot h'(x)$.
[fn:8] [Bollobas, Riordan - 2005]
[fn:9] /with high probability/, ovvero con probabilità /almeno/ $1 - \left( \frac{1}{n} \right)^{\Omega(1)}$
