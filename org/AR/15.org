...
* Dimostrazione Teorema di Arrow
  Riassumendo, il teorema di Arrow ci diceva che _per ogni_ insieme \(\left[ k \right]\) di votanti esiste un \(j \in \left[ k \right]\) tale che _indipendentemente_ dalle \(n > 2\) alternative avremo che
  ...
  Nella [[./14.html][lezione precedente]] è stata dimostrata la prima delle tre parti della dimostrazione del teorema.
...
** Parte 2 dimostrazione
   Definiamo una successione di \(k+1\) profili \(P^{(0)}, P^{(1)}, P^{(k)} \in \Pi(\left[ n \right])^k\), dove in ciascuno dei quali una stessa alternativa \(x \in \left[ n \right]\) è _polarizzante_.
   Tramite essi individueremo un /dittatore potenziale/.
   
   #+ATTR_HTML: :width 100%
   [[file:../images/ar-lesson15-img1.png]]

   Consideriamo tali profili ordinati in modo tale che nel profilo \(P^{(h)} = \langle r^h_1, ..., r^h_k \rangle\) l'alternativa $x$ è al primo posto nei primi $h$ ranking, ed ultima nei restainti (come in figura).
   Più precisamente
   \[
   ...
   #+ATTR_HTML: :width 100%
   [[file:../images/ar-lesson15-img2.png]]
   ...
   Dato che dal profilo 0 al profilo $k$ l'alternativa $x$ passa da essere all'utlimo posto fino al primo, esisterà certamente un _primo_ profilo \(j \in \left[ k \right]\) per il quale $x$ non è più all'ultimo posto.
   Ovvero, per ogni \(h < j\) avremo che \(\rho^{(h)}(x) = 0\) mentre nel profilo $j$ avremo \(\rho^{(j)}(x) > 0\).\\

   Inoltre, per quanto visto nella _prima parte della dimostrazione_, dato che $x$ è polarizzante per \(P^{(j)}\) avremo che o \(\rho^{(j)}(x) = 0\) o \(\rho^{(j)}(x) = n-1\).
...
** Parte 3 dimostrazione
   ...
   Vogliamo dismotrare che _qualunque sia_ $Q$ allora \(r^Q = r^Q_j\).
   Più precisamente, _qualunque sia_ $Q$, comunque si scelgano due alternative \(y,z \in \left[ n \right]\) allora
   \[
...
*** Sottofase 3.1
    ...
    #+CAPTION: Transformazione dal profilo $Q$ al profilo $T$.
    #+ATTR_HTML: :width 100%
    #+NAME: fig:Q2T
    [[file:../images/ar-lesson15-img3.png]]

    Osserviamo che il profilo appena costruito $T$ è molto simile al profilo \(P^{(j)}\) trovato nella fase 2.

    #+ATTR_HTML: :width 100%
    [[file:../images/ar-lesson15-img4.png]]
    ...
    #+ATTR_HTML: :width 100%
    [[file:../images/ar-lesson15-img5.png]]
    ...
    Osserviamo ora la figura [[fig:Q2T]].
...
*** Sottofase 3.2
    ...
    Iniziamo con l'osserva che dato che ci sono _più di due alternative_, ovvero \(n > 2\), allora certamente esisterà un'altra alternativa \(z \neq y\) e \(z \neq x\).\\
    ...
    - Per \(h > 0\), \(T^{(h)}\) è ottenuta da \(P^{(h)}\) spostando $z$ in prima posizione per tutti i ranking \(i \leq h\), e in ultima per tutti gli altri, ovvero
      \[
    ...
    #+CAPTION: Sequenza di profili \(T^{(0)}, T^{(1)}, ..., T^{(k)}\).
    #+ATTR_HTML: :width 100%
    #+NAME: fig:profiliT
    [[file:../images/ar-lesson15-img6.png]]
    ...
    - Caso \(\ell < j\) ::
      ...
      Ricordiamo che nel punto 2 abbiamo scelto $j$ in modo tale che \(\rho^{(j-1)}(x) = 0\), ovver che
      \[
      \rho^{(j-1)}(x) < \rho^{(j-1)}(y)
      \]
      Ma per come è definita la sequenza \(P^{(0)}, P^{(1)}, ..., P^{(k)}\) sappiamo che per ogni profilo \(h\) e per ogni \(i \leq h\) abbiamo che \(\rho^{h}_i(x) = n-1\),
      perciò avremo che \(\rho^{(j-1)}_{\ell}(x) = n-1\) dato che \(\ell < j \implies \ell \leq j-1\).\\

      Perciò avremo che nel ranking \(\ell\) del profilo profilo \(P^{(j-1)}\) che
      \[
      \rho^{(j-1)}_{\ell}(x) > \rho^{(j-1)}_{\ell}(y)
      \]
      Quindi certamente \(\rho^{(j-1)} \neq \rho^{(j-1)}_{\ell}\).\\

      #+ATTR_HTML: :width 100% :style max-width: 300px;
      [[file:../images/ar-lesson15-img7.png]]

      Questo stesso ragionamento si può applicare per ogni profilo minore di \(j\), perciò per forza non può essere vero che \(\ell < j\).
    
    - Caso \(\ell > j\) ::
      Per come abbiamo scelto $j$ sappiamo che \(\rho^{(j)}(x) = n-1\) e che quindi
      \[
      \rho^{(j)}(x) > \rho^{(j)}(y)
      \]
      Però per come abbiamo definito la sequenza di profili \(P^{(0)}, P^{(1)}, ..., P^{(k)}\), sappiamo che per ogni \(i > h\) abbiamo che \(\rho^{h}_i(x) = 0\).
      Perciò dato che \(\ell > j\) allora
      \[
