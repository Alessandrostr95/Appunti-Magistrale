...
** STCP e Local Bridges
   #+begin_quote
   *THM*
   Sia una rete \(G=(V,S\cup W)\), un nodo $u \in V$ che soddisfa la =STCP= e due suoi vicini $x,y$ tali che $(u,x) \in S$ e $(u,y)$ è un /local bridge/ (ovvero \(N(u) \cap N(y) \equiv \emptyset\) )
   ...
   *Proof*
   ...
   Ma poiché \(N(u) \cap N(y) \equiv \emptyset\) e $x \in N(u)$, allora deve essere vero che $x \not\in N(y)$ (*assurdo*)
...
** Cut-Communities
   ...
   Osservare che questo si può fare in /tempo polinomiale/[fn:1], perciò possiamo affermare che il problema decisionale che stabilisce se un grafo è partizionabile in due
   ...
   #+ATTR_HTML: :style max-width:450px; :width 100%
   [[file:../images/ar-lesson07-img2.png]]
...
** Web-Communities
   Una (*strong*) *web-community* un sottoinsieme _proprio e non vuoto_ $C \subset V$, tale che i nodi che lo compongono hanno più vicini all'interno di $C$ stesso anzichè all'esterno, ovvero
   \[
...
** Relazione tra cut- e web-communities
   ...
   *THM* Sia un grafo $G = (V,E)$.
   Se $G$ ha una cut-community $C \subset V$ tale che $|C| > 1$, allora $C$ è anche una weak web-community.
   ...
   *Proof*
   ...
   \end{align*}
   Quindi abbiamo trovato un altro taglio _minore_ \((C, V \setminus C)\), e questo è assurdo in quanto essendo $C$ per ipotesi una cut-community deve essere vero che
   ...
   Anche se esiste un algoritmo polinomiale che decide se un grafo è partizionabile in cut-community, non ne conosciamo un che _decida_ in tempo polinomiale se un grafo è
   ...
   In realtà, non solo non conosciamo un algoritmo che possa farlo in tempo polinomiale, ma è possibile dimostrare che tale problema è *NP-completo*[fn:2].\\
   ...
   *Proof* Consideriamo una partizione in due strong web-communities $C, V \setminus C = \overline{C}$, e siano $v, w$ i due soli nodi vicini di $u$.
   Supponiamo per assurdo che $v \in C$ e $w \in \overline{C}$.
   Dato che $C, \overline{C}$ è una partizione di $V$, allora necessariamente o $u \in C$ o $u \in \overline{C}$.
   Se $u \in C$ avremo che $\vert N(u) \cap C \vert = \vert N(u) \setminus C \vert$, e quindi $C$ non sarebbe una _strong_ web-community.
   ...
   *Proof (THM)*
   ...
   #+caption: Esempio gadget variabile.
   #+ATTR_HTML: :style max-width:300px; :width 100%
   [[file:../images/ar-lesson07-img3.png]]
   ...
   #+caption: Esempio gadget clausola.
   #+ATTR_HTML: :style max-width:450px; :width 100%
   [[file:../images/ar-lesson07-img4.png]]
   ...
   Inoltre poiché $c_j$ ha grado esattamente $6$, e poiché 3 dei suoi vicini sono nella comunità a cui appartiene $T$, allora basta che _almeno_ uno tra i suoi letterali (nell'esempio predente \(x_1, \overline{x_2}, \overline{x_3}\))
   appartenga anch'esso alla comunità in cui si trova $T$.\\

   Dalla costruizione appena fatta emerge che quindi $c_j$ deve appartenere alla stessa comunità a cui appartiene $T$, e per rendere ciò possibile _almeno uno_ dei letterali che lo compongono deve appartenere alla stessa comunità.\\
   ...
   [[file:../images/ar-lesson07-img5.png]]
   
  
   Riassumendo abbiamo visto che affinché $G$ sia partizionabile in due web-communities è necessario che $T$ ed $F$ siano contenuti in due
   comunità differenti, e se $T$ ed $F$ sono in due comunità differenti abbiamo che anche $x_i$ e $\overline{x_i}$ sono in due comunità differenti, per ogni $i = 1, 2, ..., n$.
   ...
   Viceversa, se esiste un'assegnazione \(a: X \rightarrow \lbrace \texttt{true}, \texttt{false} \rbrace\) che soddisfa $\phi$, allora è possibile ricavare
   una partizione di $G$ in due web-communitie.
   Innanzitutto poniamo $T$ in $C$ ed $F$ in $V \setminus C$.
   Dopodiché, dato che $a$ soddisfa $\phi$, allora insieriamo in $C$ tutti i nodi clausola $c_j$ e i rispettivi $l_{j1}, l_{j2}, l_{j3}$.
   Infine, per ogni $i$, inseiriamo in $C$ i seguenti nodi:
...
[fn:2] un problema =X= è *NP-completo* se _ogni_ problema appartenente alla classe *NP* può essere /ridotto polinomialmente/ al problema =X=.
