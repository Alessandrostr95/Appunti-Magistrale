...
Per dimostrare il teorema è prima necessario introdurre il seguente lemma
    ...
    *Proof (Lemma)* Siano $C_1, C_2, ..., C_k$ tutte le componenti connesse di $G_{n,p}$ ordinate in
    ...
    - Non ci possono essere archi nel taglio $(A,\left[ n \right] \setminus A)$, in quanto i due insiemi sono composti da componenti connesse.
      ...
      ma ciò significherebbe che $C_i \cup C_j$ è a sua volta una componente connessa (*assurdo!*).
    #+end_quote
    ...
    *Proof (THM)* Si deisdera calcolare $\mathcal{P}\left( X < \frac{n}{2} \right)$, ovvero il complementare della probabilità che si
    ...
    = \sum_{A \subset \left[ n \right] \land \frac{n}{4} \leq |A| < \frac{3}{4}n } (1-p)^{|A| \cdot |\overline{A}|}\\
    \leq \sum_{A \subset \left[ n \right] \land \frac{n}{4} \leq |A| < \frac{3}{4}n } (1-p)^{\frac{3}{16}n^2}\\
    \]
    #+end_quote
