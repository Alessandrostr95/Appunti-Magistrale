...
* Rilassare il modello
  La definizione teorica discussa fin ora era basata su assunzioni forti: un arco poteva essere un /bridge edge/ o no, e poteva rappresentare uno /strong/ o /weak tie/.
  Riportare i concetti di /bridge edges/ e /strong/waeak ties/ in una rete sociale reale è però un'assunzione _troppo forte_.
  Per esempio in una rete molto grande è molto raro trovare dei /bridge edges/, inoltre le persone non classificano le proprie relazioni in maniera dicotomica =strong/weak=,
  in genere ci sono moltissime sfumature.\\
  ...
  Quindi invece di considerare una rete \(G = (V, S \cup W)\), potremmo per esempio definire una rete \(G = (V,E,w)\) dove \(w : E \rightarrow \mathbb{N}\) è una funzione che rappresenta
  la /forza/ delle relazioni, più è alto il valore \(w(u,v)\) più la relazione tra i nodi $u$ e $v$ è _forte_.\\
  ...
  Sappiamo che i due concetti /bridge edges/ e /strong/waeak ties/ sono correllati, ovvero sappiamo che qualora una rete $G$ soddisfa la =STCP=[fn:3] allora tutti i /local bridge/ sono /weak ties/.
  Ciò che si può chiedere è se questo tipo di relazione persiste anche nel modello rilassato appena descritto.\\

  Data la generalità del modello rilassato, è più complesso andare a fare un'analisi rigorosa.
  In aiuto però esistono dei risultati di tipo empirico risultati da un esperimento di /Onnela et al/ del 2007.
  ...
  Tale rete rappresentava una rete socialre reale, in quanto i numeri telefonici erano ad uso /comune/ (e non /commericale/ o /aziendale/), e inoltre la compagnia telefonica che ha fornito i dati
  copriva una vasta area di una popolazione.
  ...
  Dato che la rimozione di archi con peso basso, rendeva più difficile la comunicazione tra gruppi di nodi, è sensato pensare che tanto più il peso di un arco è basso tanto più è bassa
  la neighborhood overlap di tale arco.\\

  #+caption: Esempio di come cresce la neighborhood di un arco \(N(u,v)\) al crescere del suo peso \(w(u,v)\). 
  #+ATTR_HTML: :style max-width:400px; :width 100%
  [[file:../images/ar-lesson08-img9.png]]
...
* Osservazioni finali
  ...
  [[file:../images/ar-lesson08-img10.png]]
...
[fn:3] =Strong Triadic Closure Property=
[fn:4] J.-P. Onnela, J. Saramaki, J. Hyvonen, G. Szabo, D. Lazer, K. Kaski, J. Kertesz, and A.-L. Barabasi. Structure and tie strengths in mobile communication networks.
