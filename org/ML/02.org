...
** Prediction Risk
   ...
   L'*errore di predizione* di una predizione \(h(x)\) è ricavata semplicemente applicando la /loss function/
   \[
   ...
   \]
   \\
   Il problema di queste definizioni è che generalmente non sono noti ne \(f\) ne la distribuzione  \(\mathscr{D}_2\) di \(p_{\mathscr{D}_2}(y \vert x)\).
   Per risolvere questo incoveniente, basta calcolare il fattore di rischio rispetto al variare del solo elemento $x$.\\

   Per comincire, osserviamo che nel caso in cui $y$ è deterministicamente definita da $f$, essa è comunque dipendente da $x$.
   Perciò possiamo calcolare il valore medio al variare del solo $x$ nel seguente modo
   \[
   ...
   \[
   \mathscr{R}(h) = \mathbb{E}_{\mathscr{D}_1,\mathscr{D}_2} [ L(h(x), f(x)) ] = \int_{\mathscr{X}}\int_{\mathscr{Y}} L(h(x), f(x)) \cdot p_{\mathscr{D}_2}(f(x) \vert x) \, df(x) \, dx
   \]
   \\
   A questo punto, riscritto tutto in funzione del solo paramtro $x$, proseguiamo osservando che le uniche informazioni certe, relative a \(\mathscr{D}_1,\mathscr{D}_2\) o $f$, ce le abbiamo rispetto al training set \(\mathscr{T}\).
   Perciò empiricamente possiamo impiegare una media aritmetica rispetto alle sole informazioni di \(\mathscr{T}\)
   \[
