...
*** Terminazione e time complexity
    ...
    *THM* Sia la rete $G$ con $n$ nodi e sorgente $s$ di eccentricità $D$.
    Sotto le precedenti assunzioni il protocollo =RR= *completerà* il suo task in esattamente \(nD\) time slots e tutti i nodi *termineranno* l'esecuzione del protocollo in \(O(n^2)\) time slots.
