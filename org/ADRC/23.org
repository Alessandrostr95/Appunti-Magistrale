...
*** Algoritmo monotono
    Un algoritmo \(g()\) per un problema /one-parameter di minimizzazione/ si dice *monotono* se per ogni agente $i$,
    ...
    *THM 1* /(Mayerson ‘81)/\\
    ...
    Ovvero dato un problema =OP=, un relativo meccanismo $M$ _veritiero_ per tale problema ha necessariamente \(g()\) monotona.
    #+end_quote

    #+begin_quote
    *Proof:*
    Supponiamo per assurdo di avere un meccanismo $M$ truthfull per un problema =OP= di minimizzazione, con algoritmo \(g()\) _non monotono_.
    ...
    *Caso $i$ non sincero*\\
    ...
    - \(\Delta p \leq A\) ::
      ...
      Infatti se $i$ fosse sincero dichiarando \(r_i = t_i = x\) avrebbe un pagamento certo pagamento \(p_i(r_{-i}, x)\), mentre se dichiara di più \(r_i = y > x = t_i\),
    ...
    - \(\Delta p \geq A+k\) ::
      ...
      Infatti se $i$ fosse sincero dichiarando \(r_i = t_i = y\) avrebbe un pagamento certo pagamento \(p_i(r_{-i}, y)\), mentre se dichiara di meno \(r_i = x < y = t_i\),
    ...
    Perciò se $M$ è /truthfull/ con funzione \(g()\) non monotona, allora incorreremo in uno schema di pagamento tale che \(\Delta p \leq A\) e \(\Delta p > A\) (*assurdo!*) \(\square\).
    #+end_quote
